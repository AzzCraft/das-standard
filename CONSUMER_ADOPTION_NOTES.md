...
This note records how downstream repos should consume the governed `das-standard` v1.4.0 release bundle without rewriting the public contract.
...
- `cadence-core`
  - Uses the v1.4.0 profile/cadence family as the policy/runtime alignment target.
  - Should read canonical cadence/profile definitions when enforcing runtime behavior.

## Adoption rules

- Do not change the public manifest shape from `v1.4.0`.
