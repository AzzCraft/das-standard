# DAS Standard Context Pack (v1.4.0)
...
- Scope: Read-optimized quick reference for humans and AI agents working from the DAS v1.4.0 release bundle.
- Parent Master Doc: SPECIFICATION.md

## 1. What DAS is (in one paragraph)

DAS is a unified engineering methodology for AI-enabled products. It treats docs like code (versioned, reviewed, verified) and treats anything crossing a boundary (repo, service, storage, or release artifact) as a governed contract surface with schema + semantics + fixtures + executable checks. In v1.4.0, the standard itself also ships as a repo-canonical release bundle with machine-readable manifests, verification metadata, and release snapshots.
...
The v1.4.0 release bundle adds governed machine-readable artifacts at the repo root:
