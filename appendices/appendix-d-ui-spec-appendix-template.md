...
<a id="appendix-d-ui-spec-template"></a>

### UI Spec Appendix — Template (v1)
...
- **Scope**: UI/UX behavior, workflow-to-screen mapping, interaction states, contracts, and test hooks for `{{PROJECT_OR_SURFACE}}`.
- **Parent Master Doc**: `{{MASTER_DOC_PATH}}`
- **Interaction profile**: `{{standard_ui|hfvi_canvas_webgl_game}}`
- **Owner details**: PM `{{OWNER_PM}}`, Design `{{OWNER_DESIGN}}`, Eng `{{OWNER_ENG}}`
...
|---|---|---|---|---|---|---|---|---|
| `UI-001` | `{{SCREEN_NAME}}` | `web|h5|admin|mobile` | `static|streaming|async_job|optimistic` | `{{ROUTE_ID}}` | `W1, W3` | `F-001` | `[[FIGMA_FRAME_LINK]]` | `{{NOTES}}` |
...
|---:|---|---|---|---|---|---|---|
| 1 | `{{INTENT}}` | `UI-001` | `{{CLICK/TYPE/...}}` | `{{API call / job create}}` | `API-001, ID-003` | `{{UI_STATE}}` | `{{UI_STATE}}` |
...
**Purpose / user value**  
`{{WHAT_USER_IS_DOING_HERE}}`

**AI interaction mode**  
`{{static|streaming|async_job|optimistic}}`
...
|---|---|---|---|---|
| `S0` | Loading | `{{...}}` | ``assets/ui/{{YYYYMMDD}}/UI-001__loading.png` (example asset path)` | `{{NOTES}}` |
| `S1` | Empty | `{{...}}` | ``assets/ui/{{YYYYMMDD}}/UI-001__empty.png` (example asset path)` | `{{NOTES}}` |
| `S2` | Ready | `{{...}}` | ``assets/ui/{{YYYYMMDD}}/UI-001__ready.png` (example asset path)` | `{{NOTES}}` |
| `S3` | Error | `{{...}}` | ``assets/ui/{{YYYYMMDD}}/UI-001__error.png` (example asset path)` | `{{ERROR_CODE/MSG_RULES}}` |
...
| State ID | Name | When shown | Visual reference | Acceptance notes |
|---|---|---|---|---|
| `A0` | Thinking / Working | backend is compute-bound (RAG/tool calls/composition) | ``assets/ui/{{YYYYMMDD}}/UI-001__thinking.png` (example asset path)` | MUST be distinct from simple network loading |
| `A1` | Streaming | partial output is arriving | ``assets/ui/{{YYYYMMDD}}/UI-001__streaming.png` (example asset path)` | Define append vs replace behavior + scroll anchoring |
...
|---|---|---|---|---|---|---|
| `{{BUTTON_NAME}}` | `{{data-testid}}` | click | `{{RULES}}` | `{{API/JOB/EVT}}` | `API-001` | `{{NOTES}}` |
...
|---|---:|---|---|---|
| `{{YYYY_MM_DD}}` | `{{VERSION}}` | `{{AUTHOR}}` | `{{SUMMARY}}` | `[[PR/ADR]]` |
...
|---|---|---|:---:|---|---|
| `UIQ-001` | `{{QUESTION}}` | `{{OWNER}}` | Y/N | `{{DATE}}` | `{{NOTES}}` |
