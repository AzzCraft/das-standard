...
|---|---|---|:---:|:---:|---|
| Design system / tokens | Shared UI surfaces use approved design tokens for typography, spacing, radius, shadow, and colors | Ad-hoc hex colors, inconsistent spacing/radius, mixed typography styles | P0 | Yes | Token lint in L1/L2 and for shared UI kit; bounded exceptions only for L0 prototypes per §4.5.1 |
...
2) Unit tests for UI logic and state.
3) Accessibility checks (axe) on critical flows (keyboard + labels + focus + contrast).
4) Minimal E2E smoke for critical workflows, traceable to the UI Spec Appendix (Appendix D).

**Full / nightly or pre-release (`verify --full`)**
5) **UI kit visual contract** (when a UI kit exists): stories + snapshot baselines for all spec-defined component states.
6) Screen-level visual regression for critical screens or high-trust surfaces.
7) (Advanced) Design-to-code traceability check (UI-### ↔ named design frames) to detect orphaned specs.
